...
#naive implementation - has to run through all values for min_i <= (i,j) <= max_i
...
max_palindrome = 0
count = 0
min_i = int(pow(10, args.length-1))
min_j = int(pow(10, args.length-1))
...
for i in range(from_max,min_i, -1):
    for j in range(from_max,min_j,-1):
        count = count+1
